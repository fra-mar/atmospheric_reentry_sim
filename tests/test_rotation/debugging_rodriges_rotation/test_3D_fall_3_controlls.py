...
vel_init = np.array([[1.01,7500,-50.01]]) # vx 2 m/s backwards and 4 m/s upwards

vel_after = np.array([[1.01,7500.01,-100.01]])
...
theta = 0 #angle in radians. angle intended to turn the lift vector about velocity axis

x,y = 0,0 # x NS axis, y WE axis...in a 3D aixs

h = 1e3*40; graph_lim = h #meters #h similar to z ina 3D axis
...
while h > 10:
    ...
    ax_xy.set_xlim([-100,graph_lim*10]); ax_xy.set_ylim([-graph_lim/4,graph_lim/4])
    ...
    ax_xy.text(1,1600, vertical_speed)
    ax_xy.text(1,1400, ground_speed)
    ax_xy.grid()
    ...
    vectors = [ (vel_after_u,'red'),
               (v_lift_u, 'black'), (v_lift_r_u, 'blue')]
