...
'''
vectors as [height,distance,sideways]
'''
import numpy as np
...
def ortovector_u (v): 
    ortovector = np.array([ -v[1],v[0],0])
    orto_u = ortovector/ module (ortovector)
    return orto_u
...
vel_init = np.array([[1000,0.1]]) # vx 2 m/s backwards and 4 m/s upwards
