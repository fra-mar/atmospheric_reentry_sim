...
rho = 1.225 #air density, standard atmosphere
...
fig = plt.figure('Spheroid (=not perfect sphere) free fall', figsize = (6,6))

ax_h = fig.add_subplot(1,1,1)

ax_h.set_title ('Mass:{:} Kg.  Vel_initial:{}m/s horizontal.  L/D ratio{:.2f}'.
                format(m, v_x, L_D_ratio), fontsize = 11)
ax_h.set_ylabel ('Altitude (m)')
ax_h.set_xlabel ('Ground distance from start (m)')
ax_h.set_xlim([-50,graph_lim+1000]); ax_h.set_ylim([0,graph_lim+1000])
ax_h.grid()

ship, = ax_h.plot([],[], 'bo')
time_txt = ax_h.text(0.75,0.65,'', fontsize= 12, transform=ax_h.transAxes)
ax_h.text(0.75, 0.45,
          'Press z or x for\nbody rotation\nThis inverts\nthe Lift vector',
